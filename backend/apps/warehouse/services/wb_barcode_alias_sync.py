...
def _sync_aliases_from_orphan_orders(
  ...
  missing = [normalize_barcode(code) for code in orphan_codes if normalize_barcode(code)]
  missing = [code for code in missing if code not in index]
  if missing:
    try:
