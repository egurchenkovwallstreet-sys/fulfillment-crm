...
from apps.accounts.tenant import get_off_crm_shipment_for_user, get_seller_for_user, sellers_for_user
...
class OffCrmShipmentDeductView(APIView):
  ...
  def post(self, request, shipment_id: int):
    get_off_crm_shipment_for_user(request.user, shipment_id)

    try:
      result = deduct_off_crm_shipment(shipment_id, user=request.user)
...
class OffCrmShipmentSkipView(APIView):
  ...
  def post(self, request, shipment_id: int):
    ...
    try:
      result = skip_off_crm_shipment(shipment_id, user=request.user)
    except OffCrmShipmentError as exc:
      return Response({"detail": str(exc)}, status=status.HTTP_400_BAD_REQUEST)
