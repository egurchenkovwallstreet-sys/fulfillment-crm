"""Биллинг отгрузок WB — только заказы с напечатанным FBS-стикером в CRM."""
...
def load_weekly_shipped_orders(seller: Seller, *, weeks: int = SHIPMENTS_WEEKS_HISTORY) -> dict:
  """
  Заказы с напечатанным FBS-стикером в CRM — по факту списания остатка (StockOperation).
  Не использует поставки WB (done): в статистику не попадают заказы без печати в CRM.
  """
  today = today_local()
  ...
  charge_amounts = {
    int(row["wb_order_id"]): row["amount"]
  ...
  for op in ops:
    ...
    op_date = timezone.localtime(op.created_at).date()
    daily_counts[op_date] += 1

    if wb_order_id is not None and wb_order_id in charge_amounts:
      daily_amounts[op_date] += charge_amounts[wb_order_id]
    else:
      ...
      if unit_price is not None:
        daily_amounts[op_date] += unit_price
