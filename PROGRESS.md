...
**Дата последнего обновления:** 14.09.2026  
**Общий статус:** 📦 Операционный MVP + Ozon FBS + **кабинет владельца** (`/owner`)  
**Стадия:** **🔨 Следующая задача:** §12.5 тарифы по литражу (согласовано 06.09.2026). Инвентаризация и приёмка ✅. **14.09** — фиксы доставки WB + статистика отгрузок по баркодам ✅.
...
| **Сборка FBS долго открывается / белый экран** | ✅ **Решено** | Кэш sessionStorage + UI сразу; sync WB/Ozon в фоне через 0.5–0.6 с |
| Старый UI после деплоя | ℹ️ | `git pull && bash scripts/deploy.sh` + Ctrl+F5 |
