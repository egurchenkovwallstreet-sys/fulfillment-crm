...
**Стадия:** **🟡 Сборка FBS:** быстрое открытие экрана + фоновая синхронизация WB/Ozon. Окно ЧЗ — проверить на проде.
...
| §7 Сборка и печать этикеток FBS | 🟡 | ЧЗ, панели очереди; **окно ЧЗ:** lock UI + match заказа с маркировкой (ожидает Ctrl+F5 на проде) |
...
### Приоритет 0 — блокер операционки
0. [ ] **Сборка WB «На сборке»:** после скана баркода товара с ЧЗ **автоматически** открывается окно «Сканируйте Честный знак» (режим «Скан»). Сейчас поле баркода сбрасывается. Детали: `тз и прогрес/2026-08-30-сборка-fbs-окно-чз.md`.
