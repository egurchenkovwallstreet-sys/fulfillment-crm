...
**Стадия:** **🔨 Следующая задача:** §12.5 тарифы по литражу (согласовано 06.09.2026). Инвентаризация и приёмка ✅. **16.09** — Chrome kiosk autoprint (белый экран) ✅; синхронизация scanDt поставок WB ✅.
...
| §7 Сборка и печать этикеток FBS | ✅ | Три счётчика (На сборке / Готовые / Ошибки ЧЗ); без автопечати; reprint с confirm |
| §8 Честный знак (DataMatrix) | ✅ | Фоновая проверка 3+3 сек; панели «Ошибки»/«Без ЧЗ»; блок доставки |
| §9 Поставки и ШК | ✅ | Лист → скан → ЧЗ → QR; **scanDt** (не closedAt); заказы из ЛК WB не блокируют; **force deliver** |
| §10 Списание остатков | ✅ | При **первой печати стикера FBS**; off-CRM — панель дашборда |
| §11 Возвраты | ❌ | **Не делаем** — возврат как новая партия (приёмка / XL) |
| §12 Цены и финансы | 🟡 | **Тарифы** ✅, **frozen charges** + диалог пересчёта (`576b4c9`) ✅; **система 2 по литражу** 📋 |
| §13 Дашборды и отчёты | 🟡 | KPI WB ✅; кабинет селлера ✅; **`/owner/product-stats`** ✅; задолженность — нет |
...
| 30.08.2026 | **Сборка FBS: окно ЧЗ после скана баркода** | 🟡 | Найдены причины после режима «Лента»: `resetScanFlow` не был замком; `.first()` брал не тот заказ; лист preview ≠ saved list; карточка Ozon без ЧЗ. Ожидает проверки на проде |
| 30.08.2026 | **Лист подбора при смене склада + печать после ЧЗ** | 🟡 | Склад: пересборка/preview, не старый PDF. ЧЗ: не ждать 5с мост печати, не стирать DataMatrix |
| 03.09.2026 | **Изоляция складов, инвентаризация, приёмка set_actual** | ✅ | `ef3f621`, `2e037ce`, `147c01c`, `644d8a9`, `38e57b5` |
| 04.09.2026 | **Сборка FBS: очередь Готовые, без повторной печати** | ✅ | `95555b6`, `699d78e`, `4d6386b`; см. `тз и прогрес/2026-09-04-сборка-fbs-очередь-готовые.md` |
| 14.09.2026 | **Тариф: frozen charges + диалог «пересчитать / с сегодня»** | ✅ | `576b4c9`, `7f4a64b`; `ShipmentUnitCharge`, `unit_billing` |
| 14.09.2026 | **Статистика отгрузок по баркодам (`/owner/product-stats`)** | ✅ | `f03c9b7`…`1322053`; источник — `StockOperation` «стикер FBS» |
| 14.09.2026 | **Фикс: closedAt ≠ scanDt — заказы не пропадают из «В доставке»** | ✅ | `5e6501c`; регрессия после `58f7909` |
| 14.09.2026 | **Фикс: заказы из ЛК WB не блокируют доставку + force deliver** | ✅ | `8f76315`; кнопка внизу «Готовые» |
| 14.09.2026 | **Ночная сверка зависших в доставке (4:00)** | ✅ | `58f7909`; Celery |
| 14.09.2026 | **Повторная печать стикера под превью FBS** | ✅ | `fdb3d49` |
| 15.09.2026 | **Chrome `--kiosk-printing`: белый экран на странице сборки FBS** | ✅ | `a287bd2`; убран `@media print`, guard main window, blob popup вместо iframe |
| 16.09.2026 | **Синхронизация scanDt поставок WB (массовая + repair)** | ✅ | `717d6c6`…`45c7855`; delivery sync, Celery 2 мин, `repair_supply_scans` |
| 16.09.2026 | **Kiosk autoprint: защита печати всегда на главной вкладке CRM** | ✅ | `9f4c258`; `beforeprint`, fix `warehouses?.length`; см. `тз и прогрес/2026-09-16-kiosk-autoprint-и-синх-scanDt.md` |
...
- **orders** — Order, Supply, PickList; sync, статусы, лист подбора, сборка, supply_flow
  ...
  - `services/supply_sync.py` — импорт поставок WB (в т.ч. из ЛК), **только scanDt** для закрытия «В доставке»; revert по closedAt
  - `services/crm_product_stats.py` — отгрузки по баркодам для `/owner/product-stats`
...
### Приоритет 0 — текущая задача (05.09.2026)
0. [x] **Инвентаризация и приёмка:** CRM = факт/было+принято; WB = CRM − «Новые»; модалка OK/расхождение. См. `тз и прогрес/2026-09-05-инвентаризация-приемка.md`.

### Следующая крупная задача — §12.5 (согласовано 06.09.2026)
- [ ] **§12.5 Тарифы по литражу (система 2):** согласовано 06.09.2026 — ежедневное хранение, ступенчатая отгрузка, 2 блока в `/cabinet`. См. `тз и прогрес/2026-09-04-тарифы-по-литражу-план.md`.
...
| **Окно ЧЗ не появляется после скана баркода** | 🟡 **В коде** | Lock UI, сверка листа/баркода. Проверить на проде после Ctrl+F5. См. `тз и прогрес/2026-08-30-сборка-fbs-окно-чз.md` |
| **Скан и лента стикеров смешивались; PDF не было на обеих вкладках; лента сбоила** | ✅ **Решено** | Явная «Сформировать лист подбора» + PDF на Новые/На сборке; лента через агент PNG по очереди |
| **Сборка FBS долго открывается** | ✅ **Решено** | Кэш sessionStorage + UI сразу; sync WB/Ozon в фоне через 0.5–0.6 с |
| **Chrome kiosk autoprint: белый экран при входе в сборку селлера** | ✅ **Решено** | `a287bd2`, `9f4c258`; не печатать main window; popup 58×40; деплой `FULL_REBUILD=1` + Ctrl+F5 на ПК |
| **Поставки WB: scanDt в CRM не подтягивается, счётчик «В доставке» не падает** | ✅ **Решено** | `45c7855`; import не сбрасывает scanDt; `repair_supply_scans`; Celery sync каждые 2 мин |
| Старый UI после деплоя | ℹ️ | `git pull && FULL_REBUILD=1 bash scripts/deploy.sh` + Ctrl+F5 |
...
| **30.08.2026** | **Окно ЧЗ: lock фазы скана, сверка баркода/листа, WB-товар (не Ozon)** | Ассистент |
| **30.08.2026** | **Скан ≠ лента: кнопка листа подбора + PDF на обеих вкладках; печать ленты через агент** | Ассистент |
| **14.09.2026** | **Доставка WB (scanDt), product-stats, тарифы frozen, force deliver** | Ассистент |
| **15–16.09.2026** | **Kiosk autoprint FBS (белый экран) + массовая синхронизация scanDt поставок** | Ассистент |
...
- `7182ed6`…`a98af0b` — FBS scan→ChZ window: perf dedupe, optimistic UI, barcode normalize, fix reset on `not_in_pick_list`
- `576b4c9` — Frozen shipment charges + tariff apply dialog
- `58f7909` — Nightly reconcile stuck in-delivery supplies (scanDt)
- `f03c9b7`…`1322053` — Owner product stats by barcode (StockOperation sticker deduction)
- `5e6501c` — Fix closedAt mistaken for warehouse scanDt
- `8f76315` — Exclude departed WB LK orders from delivery block + force deliver button
- `717d6c6` — Celery queues, background sync assembly
- `24730c7` — scanDt via supply detail API; restore delivery tab sync
- `51f1223` — Fast delivery sync mode
- `c8db8b6` — Assembly sellers in_delivery counter fix
- `50c99d4` — Mass scanDt sync every 2min (Celery beat)
- `45c7855` — Stop supply import undoing scanDt; repair_supply_scans command
- `a287bd2` — Fix white screen Chrome kiosk-printing on assembly page
- `9f4c258` — Kiosk autoprint: always block main-window print + beforeprint guard
...
**Детали 30.08.2026 (окно ЧЗ при скане баркода):** [тз и прогрес/2026-08-30-сборка-fbs-окно-чз.md](тз%20и%20прогрес/2026-08-30-сборка-fbs-окно-чз.md)  
**Детали 14.09.2026:** [тз и прогрес/2026-09-14-доставка-статистика-тарифы.md](тз%20и%20прогрес/2026-09-14-доставка-статистика-тарифы.md)  
**Детали 16.09.2026:** [тз и прогрес/2026-09-16-kiosk-autoprint-и-синх-scanDt.md](тз%20и%20прогрес/2026-09-16-kiosk-autoprint-и-синх-scanDt.md)
