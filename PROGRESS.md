...
**Общий статус:** 📦 Операционный MVP + **Приёмка в XL** для клиентов без API  
**Стадия:** Следующий шаг — **§4 UI токена WB**, затем расширение финансов (§12.4 / задолженность)
...
1. [ ] **§4 UI токена WB** — вставка персонального токена селлера в разделе «Селлеры» (инструкция в TZ §4.1). Частично уже есть в «Приёмка в XL».
2. [ ] Отдельная кнопка **отправки остатков XL на склад WB** (после полноценного клиента)
