...
**Общий статус:** 📦 Рабочий MVP: приёмка + заказы FBS + лист подбора  
**Стадия:** Следующий шаг — сборка и печать этикеток FBS (ТЗ §7)
...
| §7 Сборка и печать этикеток FBS | ❌ | Не начато |
...
| §15 API Wildberries (полный набор) | 🟡 | Заказы new ✅; остатки, этикетки, ЧЗ, поставки — нет |
...
- **orders** — Order, PickList, PickListItem; sync WB, генерация листа подбора
...
- `/orders` — заказы, sync WB, лист подбора, печать
...
1. [ ] **§7 Сборка + печать этикеток FBS** — скан баркода заказа, запрос стикера WB, печать Xprinter (< 2 сек)
2. [ ] **§15 WB API — остатки** — реализовать `sync_wb_stocks` (сейчас заглушка)
3. [ ] **§4 Селлеры** — удобный ввод/шифрование токена WB в админке, генерация логина селлера
4. [ ] **§6 Статусы заказов** — синхронизация статусов WB (`/api/v3/orders/status`)

### Далее по ТЗ
5. [ ] **§8 Честный знак** — скан DataMatrix, привязка к заказу через WB API
6. [ ] **§9 Поставки** — формирование, условия готовности, ШК поставки
