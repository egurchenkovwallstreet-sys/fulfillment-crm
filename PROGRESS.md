...
**Дата последнего обновления:** 24.08.2026 (аудит реализации)  
**Общий статус:** 📦 Операционный MVP закрыт: приёмка, склад, сборка FBS, поставки, списание, печать, кабинет селлера, тарифы, `/billing`  
**Стадия:** Следующий шаг — **§11 Возвраты**, затем UI токена WB (§4) и расширение финансов (§12.4 / задолженность)
...
1. [ ] **§11 Возвраты** — кнопка как приёмка, возврат на ту же ячейку, отдельный флаг в журнале
2. [ ] **§4 UI токена WB** — вставка персонального токена селлера без Django-admin (инструкция в TZ §4.1)
