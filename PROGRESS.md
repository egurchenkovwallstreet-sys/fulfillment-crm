...
1. [ ] **§10 Списание остатков** — автоматически при отгрузке на WB (`stock_deduction.py` есть частично)
2. [ ] **§9 UI поставок** — полный раздел: список поставок, ШК, статусы (не только flow в сборке)
3. [ ] **§7 Xprinter** — собрать `.exe` агента печати, выложить в `frontend/public/downloads/`
