...
**Стадия:** **🔴 Открытый баг сборки WB:** после скана баркода не появляется окно ЧЗ (см. `тз и прогрес/2026-08-30-сборка-fbs-окно-чз.md`). Ozon FBS шаг 5 закрыт; дальше: полировка и тесты (шаг 6).
...
| §7 Сборка и печать этикеток FBS | 🟡 | ЧЗ, панели очереди; **🔴 окно ЧЗ после скана баркода не появляется** (5+ фиксов, последний `a98af0b`) |
...
| 30.08.2026 | **Сборка FBS: окно ЧЗ после скана баркода** | 🔴 | 7 коммитов (`7182ed6`…`a98af0b`): optimistic UI, DOM/CSS, WB needKiz, fix reset — **на проде не подтверждено** |
...
| **30.08.2026** | **Документация + фиксы окна ЧЗ при скане баркода (`a98af0b`)** | Ассистент |
